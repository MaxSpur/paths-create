...
## Experimental batch serialization

`parquetWriter.ts` consumes structured `GpxTripInput` batches, sharing ordered segment assembly with `gpxWriter.ts`. It has no imports from the production UI and does not change generation or persistence. The developer browser harness loads Arrow/Parquet WASM in a disposable worker; the CLI compares exports in isolated processes. See [PARQUET_EXPORT.md](PARQUET_EXPORT.md) before integrating the prototype into batch storage or UI.
